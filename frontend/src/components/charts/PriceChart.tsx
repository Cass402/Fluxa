...
export const PriceChart: React.FC<PriceChartProps> = ({
  ...
  useEffect(() => {
    ...
    ctx.lineTo(
      scaleX(priceData[priceData.length - 1].timestamp),
    ...
    if (selectedRange) {
      ...
      ctx.textAlign = "left";
      ctx.fillText(
        `Max: ${selectedRange.max.toFixed(2)}`,
        padding.left + 5,
        rangeMaxY - 5
      );
      ctx.fillText(
        `Min: ${selectedRange.min.toFixed(2)}`,
        padding.left + 5,
        rangeMinY + 15
      );
    }
    ...
    if (showVolume && priceData.some((p) => p.volume !== undefined)) {
      ...
      for (let i = 0; i < priceData.length; i++) {
        ...
        ctx.fillStyle =
          priceData[i].price > (priceData[i - 1]?.price || priceData[i].price)
  ...
  return (
    <div className={cn("relative", className)}>
        ...
        <div className="flex space-x-1 text-sm">
          {["1H", "1D", "1W", "1M", "1Y"].map((tf) => (
            <button
              ...
              className={cn(
                "px-2 py-1 rounded",
                timeframe === tf ? "bg-primary text-white" : "hover:bg-gray-100"
              )}
              onClick={() =>
                onTimeframeChange?.(tf as "1H" | "1D" | "1W" | "1M" | "1Y")
              }
            >
              {tf}
            </button>
      ...
      <div className="mt-2 flex justify-between text-xs text-gray-500">
        <span>
          Last Price: {priceData[priceData.length - 1].price.toFixed(2)}
        </span>
        <span>
          24h Change:
          {priceData.length > 1 && (
