...
/// Provides mathematical utilities for price and liquidity calculations in a concentrated liquidity AMM.
///
/// # Methods
///
/// - `calculate_amount_0_delta`: Calculates the amount of token0 required for a given liquidity between two square root price boundaries.
/// - `calculate_amount_1_delta`: Calculates the amount of token1 required for a given liquidity between two square root price boundaries.
/// - `calculate_amounts_for_liquidity_piecewise`: Calculates the required amounts of token0 and token1 for a given liquidity position, based on the current, lower, and upper square root price boundaries.
/// - `calculate_position_value_at_price`: Calculates the USD value of a liquidity position at a given current square root price.
///
/// # Errors
///
/// All methods return a `Result` and may return errors in the following cases:
/// - Invalid price range or input values (e.g., lower price not less than upper price, zero values).
/// - Arithmetic overflows or underflows during calculations.
/// - The resulting token amounts or values exceed the maximum allowed value.
/// - Tick-to-sqrt conversion fails for the position's tick boundaries.
///
/// # Usage
///
/// These methods are intended to be used for precise and safe calculations of token amounts and position values
/// in the context of a concentrated liquidity automated market maker (AMM), such as Uniswap v3-like protocols.
///
/// # Example
///
/// ```rust
...
impl LiquidityMath {
    ...
    pub fn calculate_amounts_for_liquidity_piecewise(
            ...
            && sqrt_price_current.raw() < sqrt_price_upper.raw()
        {
            ...
            let amount_0 =
                Self::calculate_amount_0_delta(sqrt_price_current, sqrt_price_upper, liquidity)?;

            let amount_1 =
                Self::calculate_amount_1_delta(sqrt_price_lower, sqrt_price_current, liquidity)?;

            (amount_0, amount_1)
        } else if sqrt_price_current.raw() <= sqrt_price_lower.raw() {
            // If the current price is below the lower boundary, only token0 is required
            let amount_0 =
                Self::calculate_amount_0_delta(sqrt_price_lower, sqrt_price_upper, liquidity)?;

            (amount_0, 0)
        } else {
            // If the current price is above the upper boundary, only token1 is required
            let amount_1 =
                Self::calculate_amount_1_delta(sqrt_price_lower, sqrt_price_upper, liquidity)?;
    ...
    fn calculate_position_value_at_price(
        ...
        let (amount_0, amount_1) = Self::calculate_amounts_for_liquidity_piecewise(
            current_sqrt_price,
            sqrt_price_lower,
            sqrt_price_upper,
            position.liquidity,
